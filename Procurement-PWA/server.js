...
app.get('/documents/:id/download', async (req, res) => {
    ...
    try {
        ...
        const hasAccess = await checkDocumentAccess(doc, user.id, user.role, user.department_id);
...
app.get('/documents/:id/stream', ensureAuthenticated, async (req, res) => {
    try {
        ...
        const hasAccess = await checkDocumentAccess(doc, userId, userRole, deptId);
        if (!hasAccess) {
...
app.post('/upload', ensureAuthenticated, (req, res) => {
    ...
    upload.single('document')(req, res, async (err) => {
        ...
        try {
            ...
            // Document versioning: if this upload is a corrected resubmission of a
            // rejected document, link it into that document's version chain.
            let versionGroupId = null;
            let versionNumber = 1;
            const resubmitOf = req.body.resubmitOf || null;
            if (resubmitOf) {
                try {
...
// Document versioning: only the newest version in each chain should show
// up in list views (my-inbox/my-outbox/filter-options) — older, superseded
// versions stay individually reachable via GET /documents/:id/versions,
// but shouldn't clutter the active lists. No bound params needed, so this
// can just be pushed straight into a `conditions` array.
const LATEST_VERSION_ONLY_CLAUSE = `NOT EXISTS (
    SELECT 1 FROM documents newer
    WHERE COALESCE(newer.version_group_id, newer.id) = COALESCE(d.version_group_id, d.id)
